...
with sync_playwright() as p:
    ...
    ids=page.evaluate("""()=>{const v=Object.values(window.__GWENT_PASS10__.engine.CARD_DB),find=(fn)=>v.find(fn)?.id;return{
      ...
      muster:find(d=>d.type==='unit'&&d.abilities.includes('muster')&&d.row!=='agile'),
    ...
    s=empty_state(base);s['round']=2;s['players']['p1']['faction']='skellige';s['players']['p2']['faction']='monsters';s['players']['p2']['passed']=True;s['players']['p1']['passed']=False;s['currentPlayerId']='p1';s['players']['p1']['board']['close']=[I(f'win{i}',ids['ordinary']) for i in range(3)];s['players']['p2']['board']['close']=[I(f'mon{i}',ids['ordinary']) for i in range(2)];s['players']['p1']['grave']=[I('sk-a',ids['ordinaryRanged']),I('sk-b',ids['ordinarySiege'])];reset(page,s);shot(page,'57_round23_pre.png');page.locator('#pass-button').click();wait_stage(page,'round-end',9000);page.wait_for_function("document.querySelectorAll('.gc-snapshot-round-hold').length>=5",timeout=6000);shot(page,'58_round23_hold.png');wait_idle(page,15000);final=state(page);assert final['round']==3 and final['players']['p2']['retainedIid'];revived=[c['iid'] for r in ['close','ranged','siege'] for c in final['players']['p1']['board'][r] if c['iid'] in ['sk-a','sk-b']];assert len(revived)==2,revived;last=page.evaluate('window.GwentGameplayChoreography.runtime.lastPlan');round_stage=next(x for x in last if x['kind']=='round-end');assert len(round_stage['factionRevives'])==2 and round_stage['retention']['retained'];clean(page);shot(page,'59_round23_final.png')
