...
with sync_playwright() as p:
    ...
    # 1. Crowded Decoy row: target remains in 10.3 absolute geometry, one card locks,
    # and only its immediate local neighbors yield by presentation transform.
    ...
    results.append({'case':'crowded-lock','snapshot':snap})

    # 2. Move directly to another explicit target: the lock transfers and the old local
    # yield is fully reversed before the new neighborhood responds.
    ...
    results.append({'case':'lock-transfer','snapshot':snap2})

    # 3. Leave the target row: predictive state disappears and every neighbor returns
    # to untouched 10.3 resting geometry. Re-enter proves reversibility.
    wb=page.locator('#match-screen .weather').bounding_box();assert wb
    move_to(page,{'x':wb['x']+wb['width']/2,'y':wb['y']+wb['height']/2})
    assert page.locator('.te-locked-target').count()==0
    ...
    assert page.locator('.te-locked-target,.te-neighbor-yield').count()==0
    assert_geometry_equal(base_geom,geometry(page))
    results.append({'case':'leave-reenter-cancel','restored':True})

    # 4. Runtime reduced motion preserves unmistakable target identity but performs no
    # neighbor translation or target lift.
    ...
    transform=page.locator('[data-inspect-board="qa-target-3"]').evaluate("e=>getComputedStyle(e).transform")
    ...
    results.append({'case':'reduced-motion','snapshot':rs,'transform':transform})
...
print('pass11-target-exposure-ui: localized Decoy lock/yield, frozen geometry, cleanup, reduced motion, and trajectory expiry passed')
